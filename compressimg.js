...
(function(){
        window.compressImg = function(evt,imagefiles,compreimg,quality,afterwidth){
            var self = evt.currentTarget,hidCanvas = document.createElement('canvas');
            var imgSrc = compreimg.src;
            //生成隐藏画布
            ...
            reader.onload = function(evt){
                ...
                p.onload = function(){
                    ...
                    if(orientation <= 4){
                        ...
                        // 获取压缩后生成的img对象
                        compreimg.src = convertCanvasToImage(hidCanvas,quality).src;
                        //console.log(compreimg.src.length);
                        compreimg.style.display = "block";
                        //self.value = "";
                        // 此处将得到的图片数据回调
                        //if(callback!=undefined){callback(compreimg.src)};
                    }
                }
            }
        ...
        function convertCanvasToImage(canvas,quality) {
          var image = new Image();
          image.src = canvas.toDataURL("image/jpeg",quality/100);
          return image;
        }
        /**
